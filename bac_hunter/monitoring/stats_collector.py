...
class StatsCollector:
    ...
    def record_request(self, url: str, method: str, status_code: int, response_time_ms: float, response_size: int, identity: str = "unknown"):
        with self.lock:
            stats = RequestStats(
                timestamp=time.time(),
                url=url,
    ...
    def get_summary(self) -> Dict[str, Any]:
        with self.lock:
            ...
            return {
                ...
                'top_status_codes': dict(sorted(self.status_codes.items(), key=lambda x: x[1], reverse=True)[:5]),
                'findings_by_type': dict(self.findings_by_type),
                'hosts_tested': dict(self.hosts_tested),
                'success_rate': (self.scan_stats.successful_requests / max(1, self.scan_stats.total_requests)) * 100,
                'rate_limit_rate': (self.scan_stats.rate_limited_requests / max(1, self.scan_stats.total_requests)) * 100
            }
