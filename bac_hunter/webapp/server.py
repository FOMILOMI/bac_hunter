...
async def run_scan(target: str = Query(..., description="Target base URL")):
    ...
    try:
        with _db.conn() as c:
            _db.ensure_target(target)
        # minimal quick scan
        for p in (
            RobotsRecon(settings, http, _db),
            SitemapRecon(settings, http, _db),
            JSEndpointsRecon(settings, http, _db),
            SmartEndpointDetector(settings, http, _db),
        ):
            try:
                tid = _db.ensure_target(target)
