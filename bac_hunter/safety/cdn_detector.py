...
class CDNDetector:
    """Detects CDN usage to avoid aggressive testing"""
    ...
    def detect_cdn(self, response_headers: dict) -> Optional[str]:
        ...
        # Check server header for common CDN signatures
    ...
    def should_throttle_more(self, cdn_type: Optional[str]) -> bool:
        """Determine if we should use extra conservative rate limiting"""
        if not cdn_type:
            return False
            
        # These CDNs are more aggressive with rate limiting
