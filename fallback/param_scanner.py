...
class ParamScanner:
    ...
    Targets the base URL and key known endpoints from DB.
    Records potentially interesting deltas (e.g., 200/403, sizable bodies).
    """
    ...
    async def run(self, base_url: str, identity: Identity) -> List[str]:
        urls = []
        # Use base_url plus a few URLs already known for that target
